...
public class YouTubeTitleParserTests
{
    ...
    // Without a channel, "Sing King Karaoke" as a pipe segment is indistinguishable from real
    // content ("The Steve Miller Band Karaoke Version" has the same shape), so these need the real channel.
    ...
    // so this channel stays Artist-first and "(Piano Karaoke)" is real leftover bracket content.
    ...
    // Rule 2: "•" as a separator; also exercises the OverlapsChannel leftover-length guard so a
    // "•"-delimited bracket does not swallow the title. "[UVR]" is not junk, so it survives.
    [InlineData("Led Zeppelin  • Stairway To Heaven (CC Karaoke / Instrumental) [UVR]", "CC Karaoke", "Stairway To Heaven [UVR]", "Led Zeppelin")]
    // OverlapsChannel: a trailing segment naming the channel is dropped whole, even when
    // decoration outlasts the channel name; Rule 3's junk stripping already covers that case.
    ...
    // "Title Karaoke | Artist Karaoke Version" is Vocal Star Karaoke's (PipeTitleFirstChannels).
    ...
    // name has a space; IsPipeSegmentJunkOrChannelPromo folds hyphen/space so this is still dropped.
    ...
    public void Parse_ChannelOverlapSegment_IsNotDroppedWhenItIsTheOnlySegmentLeft()
    {
        ...
        // segment left once "Karaoke Version" is stripped as junk, so the guard has to keep it.
    ...
    // appearing once. A channel allowlist cannot chase that tail; the same search's other results can.
    [Fact]
    ...
    // Both halves of this row are anchors: the artist because another result stated it, the
    // title because two backwards results made it modal. A row with a known artist is not swapped.
    [Fact]
